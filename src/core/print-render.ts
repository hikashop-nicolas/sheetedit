...
export interface Pagination {
  /** Column groups and row groups; a page is one of each, combined in page order. */
  colPages: number[][];
  rowPages: number[][];
...
export function paginate(sheet: Sheet, setup: PrintSetup, geom: PageGeom): Pagination {
  ...
  const area = areas[0];
  if (!area) return { colPages: [], rowPages: [], scale: 1, titleRows: [], titleCols: [] };
  ...
  // Title lines are repeated on every page, so they are not part of the body run.
  const cols: number[] = [];
  ...
  // Fit-to overrides the stated scale: shrink until the content spans the requested page count.
  let scale = (setup.fitToPage ? 100 : setup.scale ?? 100) / 100;
  if (setup.fitToPage) {
    const totalW = repeatW + cols.reduce((a, c) => a + colWidth(sheet, c), 0);
    const totalH = repeatH + rows.reduce((a, r) => a + rowHeight(sheet, r), 0);
    const fits: number[] = [];
  ...
  return {
    colPages: splitLines(cols, (c) => colWidth(sheet, c), capW, new Set(setup.colBreaks ?? []), repeatW),
    rowPages: splitLines(rows, (r) => rowHeight(sheet, r), capH, new Set(setup.rowBreaks ?? []), repeatH),
    scale,
    titleRows,
    titleCols,
  };
...
/** The pages, in the order the setup asks for. */
export function pageOrder(p: Pagination, order: PrintSetup["pageOrder"]): { cols: number[]; rows: number[] }[] {
  ...
  if (order === "overThenDown") {
    for (const rows of p.rowPages) for (const cols of p.colPages) out.push({ cols, rows });
  } else {
    for (const cols of p.colPages) for (const rows of p.rowPages) out.push({ cols, rows });
  }
  return out;
}
...
/**
 * Build the printable document for a sheet: one absolutely-sized box per page, each carrying its
 * own header, body and footer. Returns the root to append, or null when there is nothing to print.
 */
export function buildPrintDocument(wb: Workbook, sheetIndex: number, fileName: string): HTMLElement | null {
  ...
  const setup: PrintSetup = sheet.printSetup ?? {};
  const geom = pageGeom(setup);
  const pagination = paginate(sheet, setup, geom);
  const pages = pageOrder(pagination, setup.pageOrder);
  if (!pages.length || !pagination.rowPages.some((p) => p.length) || !pagination.colPages.some((p) => p.length)) return null;
  ...
  // then ours to place, which is the only way to honour the file's own values.
  const style = document.createElement("style");
  style.textContent =
    `@page { size: ${Math.round(geom.pageW / CSS_DPI * 100) / 100}in ${Math.round(geom.pageH / CSS_DPI * 100) / 100}in; margin: 0; }`;
  root.appendChild(style);

  pages.forEach((page, i) => {
    const ctx = { page: (setup.firstPageNumber ?? 1) + i, pages: pages.length, sheet: sheet.name, file: fileName };
