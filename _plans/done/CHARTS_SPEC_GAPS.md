...
All four tiers of CHARTS_SPEC_GAPS are complete, and the earlier niceties are closed too:
- Date axes render on a real Chart.js time scale (dependency-free native-Date adapter in date-adapter.ts), ticks snapped to date boundaries.
- Multi-level category axes render tiered (outer levels drawn as grouped boxed rows below the innermost labels).
- Pseudo-3D (isometric) rendering covers 3-D column, bar, line, area and pie/doughnut, matching Excel's look; all still re-emit their 3-D element + view3D on write. Only radar-3D (not a real Excel type) stays flat.
Charts are feature-complete against this doc with no known caveats.
